import { Address, BillingAddress, CartChangedError, CheckoutSelectors, CheckoutSettings, OrderRequestBody, PaymentMethod } from '@bigcommerce/checkout-sdk';
...
class Payment extends Component<PaymentProps & WithCheckoutPaymentProps & WithLanguageProps, PaymentState> {
    ...
    async componentDidMount(): Promise<void> {
        ...
        const ccNumberField = document.getElementsByClassName('form-field--ccNumber');
        const ccExpiryField = document.getElementsByClassName('form-field--ccExpiry');
        const ccNameField = document.getElementsByClassName('form-field--ccName');
        const ccCVVField = document.getElementsByClassName('form-ccFields-field--ccCvv');

        if (ccNumberField && ccNumberField.length) {
            ccNumberField[0].classList.add('floating-label');
            // const cardInput = document.getElementById('card-number');
        }
        if (ccExpiryField && ccExpiryField.length) {
            ccExpiryField[0].classList.add('floating-label');
            // const cardInput = document.getElementById('card-expiry');
        }
        if (ccNameField && ccNameField.length) {
            ccNameField[0].classList.add('floating-label');
            // const cardInput = document.getElementById('card-name');
        }
        if (ccCVVField && ccCVVField.length) {
            ccCVVField[0].classList.add('floating-label');
            // const cardInput = document.getElementById('card-code');
        }
        const {
        ...
        // const backButton = document.getElementById('checkout-billing-back');
        // if (backButton) {
    ...
    componentDidUpdate(): void {
        ...
        const openNode = document.getElementById('247opennodepayment');
        if (openNode) {
            const paymentDiv = document.getElementById('checkout-view-header-payment');
            if (paymentDiv) {
                paymentDiv.appendChild(openNode);
            }
        }
    }
    ...
    componentWillUnmount(): void {
        const openNode = document.getElementById('247opennodepayment');
        if (openNode) {
            const checkoutDiv = document.getElementById('checkout-view-header-checkout');
            if (checkoutDiv) {
                checkoutDiv.appendChild(openNode);
            }
        }
    ...
    render(): ReactNode {
        ...
        let style = { pointerEvents: 'none', opacity: 0.7 };
        ...
        return (
            <PaymentContext.Provider value={ this.getContextValue() }>
                <LoadingOverlay
                    ...
                    withOutDiv={ true }
                >
                    <div id="payment-section" style={ style as CSSProperties }>
                    <div className="form-legend-container">
                      <Legend testId="shipping-address-heading">
                      <TranslatedString
                          id={ 'payment.payment_heading' }
                      />
                     </Legend>
                    </div>
                    <div className="checkout-view-header-payment" id="checkout-view-header-payment" />
    ...
    private handleSubmit: (values: PaymentFormValues) => void = async values => {
        ...
        //   let finalShipAddress: any = {...shippingAddress};
