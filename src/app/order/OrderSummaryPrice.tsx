...
function getDisplayValue(amount?: number | null, zeroLabel?: ReactNode, freeItem?: boolean | false): ReactNode | number {
    const notYetSetSymbol = '--';

    if (!freeItem && (typeof amount === 'undefined' || amount === null)) {
        return notYetSetSymbol;
    }

    if (zeroLabel && (amount === 0 || freeItem)) {
        return zeroLabel;
    }
...
class OrderSummaryPrice extends Component<OrderSummaryPriceProps, OrderSummaryPriceState> {
    ...
    render(): ReactNode {
        ...
        const displayValue = getDisplayValue(amount, zeroLabel, freeItem);
